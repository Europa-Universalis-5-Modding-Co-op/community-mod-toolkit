# Community Mod Toolkit

A community-driven toolkit for [Europa Universalis V](https://store.steampowered.com/app/3450310/Europa_Universalis_V/) modding.

This is a companion to the [Community Mod Framework](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-framework) ([wiki](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-framework/wiki)), which provides shared in-game features like the [Community Mod Menu](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-framework/wiki/Community-Mod-Menu), Custom Alerts, and more. You will need this as a dependency if you are using the CMM Visual Editor for creating your mod menu.

**[Full Documentation (Wiki)](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-toolkit/wiki)**

## Features

| Feature | Description |
|---------|-------------|
| [Mod Template](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-toolkit/wiki/Mod-Template) | Ready-to-use project structure with metadata, thumbnails, localization starters, Workshop page template, submod example, and README. |
| [Editor and Git Configuration](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-toolkit/wiki/Editor-and-Git-Configuration) | Pre-configured `.editorconfig`, `.gitattributes`, `.gitignore`, and `.ignore` that automatically enforce EU5 formatting conventions (UTF-8 BOM, line endings, indentation). |
| [Upload Tool](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-toolkit/wiki/Upload-Tool) | Build a minimal release folder and upload to Steam Workshop. Supports dev/release separation, workshop page updates, change notes, submods, and version-gated uploads. |
| [Translation Tool](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-toolkit/wiki/Translation-Tool) | Auto-translate localization files and Workshop titles/descriptions using DeepL or Gemini. |
| [GUI Update Tool](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-toolkit/wiki/GUI-Update-Tool) | Track the vanilla GUI definitions your mod overrides and three-way merge vanilla changes into them after each game update. |
| [Release Workflow](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-toolkit/wiki/Release-Workflow) | Publish to the Steam Workshop and cut a GitHub release from GitHub Actions, on a manual run or automatically on a push or merged pull request. Separate release and dev channels. |
| [CMM Visual Editor](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-toolkit/wiki/CMM-Visual-Editor) | Browser-based visual editor for creating [Community Mod Menu](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-framework/wiki/Community-Mod-Menu) settings without scripting by hand. |

## Quick Start

1. From the root of your mod folder:
   ```bash
   curl.exe -sL https://raw.githubusercontent.com/Europa-Universalis-5-Modding-Co-op/community-mod-toolkit/toolkit-release/tools/setup.py -o setup.py
   python setup.py
   ```
2. See the [Setup](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-toolkit/wiki/Setup) and [Configuration](https://github.com/Europa-Universalis-5-Modding-Co-op/community-mod-toolkit/wiki/Configuration) wiki pages for details.

## Requirements

- **Python 3.x**
- **Git**
- **Europa Universalis V**

## Credits

`upload.py` uses [SteamworksPy](https://github.com/philippj/SteamworksPy) by Philipp J for Steam Workshop uploads and page updates.

## License

GNU General Public License v3.0 - see [LICENSE](LICENSE) for details.
